...
class GeoTransformer(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        new_data['lengths'] = data_dict['lengths']
        new_data['points'] = a
        new_data['neighbors'] = data_dict['neighbors']
        new_data['subsampling'] = data_dict['subsampling']
        new_data['upsampling'] = data_dict['upsampling']
        ref_points_c1 = (R.unsqueeze(0).expand(ref_points_c.shape[0], -1, -1) @ ref_points_c.unsqueeze(-1)).squeeze(-1)
        src_points_c1 = (R.unsqueeze(0).expand(src_points_c.shape[0], -1, -1) @ src_points_c.unsqueeze(-1)).squeeze(-1)


        feats_list, equ_list = self.backbone(feats, equ, data_dict)
        ...
        ref_equ = equ_c[:ref_length_c]
        src_equ = equ_c[ref_length_c:]
        ref_feats_c, src_feats_c, ref_equ_c, src_equ_c = self.transformer(
        ...
        ref_equ_c = ref_equ_c.squeeze(0)
        src_equ_c = src_equ_c.squeeze(0)
        output_dict['ref_equ_c'] = ref_equ_c
        output_dict['src_equ_c'] = src_equ_c
        ...
        with torch.no_grad():
            ref_node_corr_indices, src_node_corr_indices, node_corr_scores = self.coarse_matching(
                ref_feats_c_norm, src_feats_c_norm, ref_equ_c, src_equ_c, ref_node_masks, src_node_masks
            )
