...
class invencoder_in(nn.Module):
    def __init__(self, in_channels, out_channels, equ_inputchannel, equ_channels, L_in, L_out, shalldow=True, init_wigner=False, radius=0.1):
        super(invencoder_in, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.equ_channels = equ_channels
        self.basis = (L_out + 1) ** 2
        ...
        self.mid_channels = max(8, out_channels // 8)

        self.norm = nn.LayerNorm(self.mid_channels)
        self.act = nn.LeakyReLU(0.1)

        self.inv_fc1 = nn.Linear(in_channels, self.mid_channels)
        self.inv_fc2 = nn.Linear(in_channels, self.mid_channels)
        self.equ_fc1 = nn.Linear(equ_inputchannel, self.mid_channels, bias=False)
        self.equ_fc2 = nn.Linear(equ_inputchannel, self.mid_channels, bias=False)

        self.fc_vec = nn.Linear(1, self.mid_channels, bias=False)
        self.inv_fc_1 = nn.Linear(self.mid_channels, out_channels, bias=False)

        # self.fc_l0_1 = nn.Linear(self.mid_channels, self.mid_channels)
        self.gate = Gate_3j(L_in, self.mid_channels)

    def forward(self, feats, equ, points, neighbors):

        feats1 = self.inv_fc1(feats)
        feats2 = self.inv_fc2(feats)
        equ_new1 = self.equ_fc1(equ)
        equ_new2 = self.equ_fc2(equ)

        s_equ = equ_new1[neighbors[0, :]]
        t_equ = equ_new2[neighbors[1, :]]
        s_feats = feats1[neighbors[0, :]]
        t_feats = feats2[neighbors[1, :]]
        
        ori_vec = points[neighbors[0, :]] - points[neighbors[1, :]]
        # distance = torch.norm(ori_vec, dim=1)
        ori_vec = ori_vec.unsqueeze(-1)
        # vec = ori_vec.expand(-1, -1, self.mid_channels)
        vec = self.fc_vec(ori_vec)
        equ_l1 = s_equ[:, 1:, :] + vec
        equ_l0 = (equ_l1 * (t_equ[:, 1:, :] + vec)).sum(1)
        # equ_l0 = self.act(self.fc_l0_1(equ_l0))
        equ_l0 = self.act(self.norm(equ_l0 + s_equ[:, 0, :] + t_equ[:, 0, :]))
        equ_message = torch.cat([equ_l0.unsqueeze(1), equ_l1], dim=1)
        equ_l0 = equ_message[:, 0, :]

        inv_message = self.inv_fc_1((s_feats + t_feats + equ_l0))
        inv_message = self.act(inv_message)

        equ_message = self.gate(equ_message)
        return inv_message, equ_message
    
class invencoder_out(nn.Module):
    def __init__(self, in_channels, out_channels, equ_inputchannel, equ_channels, L_in, L_out, shalldow=True, init_wigner=False, radius=0.1):
        super(invencoder_out, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.equ_channels = equ_channels
        self.basis = (L_out + 1) ** 2
        ...
        self.cg1 = e3nn_tp(L_in, L_in, L_out)

        self.inv_fc_1 = nn.Linear(self.mid_channels, out_channels, bias=False)
        self.gate = Gate_3j(L_out, self.mid_channels)
    ...
    def forward(self, feats, equ, points, neighbors):       
        ...
        equ_l0 = equ_message[:, 0, :]
        inv_message = self.act(self.inv_fc_1(s_feats + t_feats + equ_l0))
...
class invencoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, equ_inputchannel, equ_channels, L_in, L_out, shalldow=True, init_wigner=False, radius=0.1):
        ...
        # self.block = []
        # self.block.append(invencoder_in(in_channels, out_channels, equ_inputchannel, equ_channels, L_in, L_out))
        # self.block.append(invencoder_out(in_channels, out_channels, equ_inputchannel, equ_channels, L_in, L_out))
        self.block1 = invencoder_in(in_channels, out_channels, equ_inputchannel, equ_channels, L_in, L_out)
        if shalldow:
            self.block2 = invencoder_in(in_channels, out_channels, equ_inputchannel, equ_channels, L_in, L_out)
        else:
            self.block2 = invencoder_out(in_channels, out_channels, equ_inputchannel, equ_channels, L_in, L_out)
        ...
        if L_out <= 1:
            if equ_inputchannel == equ_channels:
                ...
                self.shortcut2 = nn.Linear(equ_inputchannel, equ_channels, bias=False)
            self.equ_shortcut = True
        else:
            self.equ_shortcut = False

        self.ffn_equ = nn.Sequential(EquivariantLayerNormV2_channel(L_out, equ_channels),
                                    nn.Linear(equ_channels, equ_channels // 2, bias=False), 
                                    Gate_3j(L_out, equ_channels // 2),
                                    nn.Linear(equ_channels // 2, equ_channels, bias=False))

        self.equ_fc_1 = nn.Linear(self.mid_channels, equ_channels, bias=False)
        ...
        if in_channels == out_channels:
            ...
            self.shortcut1 = nn.Linear(in_channels, out_channels, bias=False)

        self.ffn_inv = nn.Sequential(nn.LayerNorm(out_channels),
                                    # GroupNorm(32, out_channels), 
                                     nn.Linear(out_channels, self.mid_channels // 2, bias=False), 
                                     nn.ReLU(),
                                     nn.Linear(self.mid_channels // 2, out_channels, bias=False))
    ...
    def forward(self, feats, equ, points, neighbors, cutoff=None, subsampling=None, print_time=False):
        N = feats.shape[0]
        ...
        inv_message, equ_message = self.block1(feats, equ, points, cutoff[0])
        inv_new.index_add_(0, cutoff[0][1, :], inv_message.to(inv_new.dtype))
        # if self.L_out > 1:
        equ_new[:, 0 : self.min_basis, :].index_add_(0, cutoff[0][1, :], equ_message.to(inv_new.dtype))

        inv_message, equ_message = self.block2(feats, equ, points, cutoff[1])
        inv_new.index_add_(0, cutoff[1][1, :], inv_message.to(inv_new.dtype))
        equ_new.index_add_(0, cutoff[1][1, :], equ_message.to(inv_new.dtype))
        
        inv_new = inv_new + self.shortcut1(feats)
        inv_new = inv_new / neighbors_new
        
        equ_new = self.equ_fc_1(equ_new)
        if self.equ_shortcut:
            equ_new += self.shortcut2(equ)
