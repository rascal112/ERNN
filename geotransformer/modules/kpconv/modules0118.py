...
class invencoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, equ_inputchannel, equ_channels, L_in, L_out, shalldow=True, init_wigner=False, radius=0.1):
        ...
        # self.inv_fc = nn.Linear(in_channels, self.mid_channels, bias=False)
        ...
        if not self.shalldow:
            # self.fc_l0 = nn.Linear(4 * self.mid_channels, self.mid_channels, bias=False)
            # self.fc_l1 =  nn.Linear(2 * self.mid_channels + 1, self.mid_channels, bias=False)
        # else:
        ...
        self.inv_fc_1 = nn.Linear(2 * in_channels + self.mid_channels, out_channels, bias=False)
        # self.inv_fc_2 = nn.Linear(self.mid_channels, out_channels)

        # self.fc_dist1 = nn.Linear
    ...
    def forward(self, feats, equ, points, neighbors, cutoff=None, subsampling=None, print_time=False):
        ...
        neighbor_num1 = torch.ones(neighbors.size(-1), device=neighbors.device)
        # neighbor_num = torch.gt(neighbor_num, 0.0).to(dtype=torch.int)
        N = feats.shape[0]
        neighbors_new = torch.ones(
            N,
            1,
            dtype=neighbor_num1.dtype,
            device=neighbor_num1.device,
        )

        # weights = self.fc_weights(feats)
        weights = feats[neighbors[0, :]]
        feats_new = self.inv_norm1(feats)
        equ_new = self.equ_norm1(equ)

        # feats1 = self.inv_fc(feats_new)
        equ_new1 = self.equ_fc1(equ_new)
        equ_new2 = self.equ_fc2(equ_new)

        N = feats.shape[0]

        s_equ = equ_new1[neighbors[0, :]]
        t_equ = equ_new2[neighbors[1, :]]
        s_feats = feats_new[neighbors[0, :]]
        t_feats = feats_new[neighbors[1, :]]
        ...
        # vec = ori_vec.expand(-1, -1, self.mid_channels)
        if self.shalldow:
            # equ_l0 = self.act(self.fc_l0(torch.
            # cat([dot1, dot2, s_equ[:, 0, :], t_equ[:, 0, :]], dim=1)))
            # equ_l0 = torch.ones_like(dot)
            equ_l0 = ((s_equ[:, 1:, :] + vec) * t_equ[:, 1:, :]).sum(1) + s_equ[:, 0, :] + t_equ[:, 0, :]
            # equ_l0 = s_feats + t_feats
            # equ_l1 = self.fc_l1(torch.cat([s_equ[:, 1:, :], t_equ[:, 1:, :], ori_vec[:, 1:, :]], dim=-1))
            equ_l1 = s_equ[:, 1:, :]
            equ_message = torch.cat([equ_l0.unsqueeze(1), equ_l1], dim=1)
        else:
            # shortcut1 = s_equ + t_equ
            # ori_vec = o3.spherical_harmonics(np.arange(self.L_in + 1).tolist(), ori_vec, False, normalization='component').unsqueeze(-1)
            equ_message = s_equ
            equ_message[:, 1:, :] += vec
            equ_message = self.cg1(equ_message, t_equ)
            # equ_message = self.gate1(equ_message)
            # # equ_message = self.cg1()
        ...
        # weights.max(dim=-1, keepdim=True)[0]
        weights = self.sigmoid(weights.max(-1, keepdim=True)[0])
        dist_weight = torch.clamp(distance / self.raduis, max=1.0).unsqueeze(-1)
        dist = weights * dist_weight
        # dist2 = self.sigmoid(self.fc_dist2(s_feats)) * dist_weight
        # dist1 = 
        equ_message = equ_message * dist.unsqueeze(-1)
        inv_message = inv_message * dist
