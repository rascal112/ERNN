...
class EquivariantLayerNormV2_channel(nn.Module):
    
    def __init__(self, lmax, channel, eps=1e-5, affine=True, normalization='component'):
        super().__init__()
        ...
        if affine:
            self.affine_weight = nn.Parameter(torch.ones(self.lmax + 1, self.channel))
            self.affine_bias = nn.Parameter(torch.zeros(self.channel))
        else:
            self.register_parameter('affine_weight', None)
            self.register_parameter('affine_bias', None)
    ...
    def forward(self, node_input, **kwargs):
        ...
        for i in range(self.lmax + 1):
            field = node_input[:, i**2:i**2 + 2*i+1, :]
            d = 2*i+1
            if i == 0:
                field_mean = torch.mean(field, dim=2, keepdim=True)
                # Subtract the mean
                field = field - field_mean
            if self.normalization == 'norm':
                field_norm = field.pow(2).sum(1)  # [batch * sample, mul]
            ...
            if self.affine:
                weight = self.affine_weight[None, i]  # [batch, mul]
                # iw += mul
                field_norm = field_norm * weight  # [batch, mul]
            
            field = field * field_norm.reshape(-1, 1, self.channel)  # [batch * sample, mul, repr]
            
            if self.affine and d == 1:  # scalars
                bias = self.affine_bias  # [batch, mul]
                # ib += mul
                field += bias.reshape(1, self.channel)  # [batch * sample, mul, repr]

            # Save the result, to be stacked later with the rest
            fields.append(field.reshape(-1, d, self.channel))  # [batch * sample, mul * repr]
